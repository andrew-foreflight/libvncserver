...
Dimensions _windowInset;
std::uint64_t _frameBufferHash = 0;
std::chrono::time_point <std::chrono::high_resolution_clock> _lastFrameTime;
std::unique_ptr <std::uint32_t[]> _remoteFramebuffer[2];
std::chrono::milliseconds _maxIntervalMilliseconds = std::chrono::milliseconds( 0 );
...
//! Return the framebuffer dimensions from the given window dimensions, accounting for insets etc.
Dimensions getFrameBufferSize( const Dimensions& window )
{
    return { window.x + _windowInset.x,
             window.y + _windowInset.y,
             window.w - _windowInset.w - _windowInset.x,
             window.h - _windowInset.y - _windowInset.h };
}

//! Return a hash value for the rectangular image area, suitable for understanding whether the image content has changed from one frame to another. Interval is every x bytes along a line, needs to be multiple of the image word size.
std::uint64_t getImageHash( const std::uint32_t *image, const Dimensions& area, std::int32_t stride, std::int32_t interval = 32 )
{
    std::uint64_t h = 0;
    
    for ( auto y = area.y; y < area.y + area.h; y ++ )
    {
        for ( auto x = area.x; x < area.x + area.w; x += interval )
        {
            auto offset = ( y * ( stride >> 2 ) ) + x;
...
PtrAddEvent(int buttonMask, int x, int y, rfbClientPtr cl)
{
    ...
    if ( window.isValid() )
    {
        rx = _frameBufferSize.x;
        ry = _frameBufferSize.y;
    }
    
    position.x = x + displayBounds.origin.x + rx;
    position.y = y + displayBounds.origin.y + ry;
...
ScreenInit(int argc, char**argv)
{
  ...
  if(displayNumber < 0) {
    ...
    _windowSize = window._dimensions;
    _frameBufferSize = getFrameBufferSize( window._dimensions );
...
//  rfbScreen->cursor = NULL;
  ...
  if(sharedMode) {
    ...
    options = CFDictionaryCreate(kCFAllocatorDefault, (const void **) keys, (const void **) values, 1, NULL, NULL);
  ...
  CGDisplayStreamRef stream = CGDisplayStreamCreateWithDispatchQueue(displayID,
                                    ...
                                    options,
                                     ...
                                     bool completeFrame = ( status == kCGDisplayStreamFrameStatusFrameComplete && frameSurface != NULL );

                                     auto now = std::chrono::high_resolution_clock::now();
                                     auto elapsed = now - _lastFrameTime;
            
                                     // The rate of new frame updates is throttled below a given interval in milliseconds.
            
                                     if ( completeFrame && elapsed < _maxIntervalMilliseconds )
                                     {
//                                         printf( "FPS limited, frame discarded after %ims\n", std::chrono::duration_cast <std::chrono::milliseconds> ( elapsed ).count() );
                                         return;
                                     }
                                     else if ( completeFrame && elapsed >= _maxIntervalMilliseconds )
                                     {
                                         _lastFrameTime = now;
                                     }
									 ...
									 if ( completeFrame ) {
                                         ...
                                         bool changed = ( _windowSize != window._dimensions );
                                         bool resized = ( _windowSize.resized( window._dimensions ) );
                                         
                                         // Has the window moved in any way since last time?
                                         ...
                                         if ( resized )
                                         {
                                             ...
                                             rfbNewFramebuffer( rfbScreen, (char*)frameBufferOne, _frameBufferSize.w, _frameBufferSize.h, bitsPerSample, 3, 4);
                                         ...
                                         */
                                         ...
                                         // Check whether the hash of the window section of interest has changed since the last frame. The stream could keep sending any
                                         // animated app at a high rate whether or not the content has changed.
                                         
                                         auto hash = getImageHash( fromBase, _frameBufferSize, lineSize );
                                         bool contentChanged = ( hash != _frameBufferHash );
                                         _frameBufferHash = hash;
                                         ...
                                         // Only pass on updated rectangles if the content appears to have changed as well.
...
int main(int argc,char *argv[])
{
  ...
  printf("Ensure the program has been given screen recording permissions in 'System Preferences'->'Security & Privacy'->'Privacy'->'Screen Recording'.\n");
  printf("Ensure the program has been given permission to control the computer in 'System Preferences'->'Security & Privacy'->'Privacy'->'Accessibility'.\n");
  printf("\n Note: if rebuilding the server application, delete the old app entry from 'Privacy->Screen Recording' and 'Privacy->Accessibility', run the app again to add a new unticked entry, tick the entries and re-run until permissions have be added for the newly rebuilt app.\n\n");
  ...
  for(i=argc-1;i>0;i--)
    ...
    } else if(strcmp(argv[i],"-inset")==0) {
        auto insets = std::string( argv[i+1] );
        auto values = split( insets, ',' );
        _windowInset = { std::stoi( values[0] ), std::stoi( values[1] ), std::stoi( values[2] ), std::stoi( values[3] ) };
    } else if (strcmp(argv[i],"-max-fps")==0) {
        auto maxFps = std::string( argv[i+1] );
        _maxIntervalMilliseconds = std::chrono::milliseconds( std::int32_t( 1000.f / std::stoi( maxFps ) ) );
    } else if (strcmp(argv[i],"-help")==0) {
        printf( "-wait4client <n>:      wait for a maximum of n seconds for a client to connect before quitting. \n"
                "-runforever:           don't quit after the first client to connect disconnects. \n"
                "-viewonly:             only send the screen, don't handle any input from the client. \n"
                "-shared:               ? \n"
                "-display <n>:          select the display (0-based), if multiple monitors are connected. \n"
                "-report:               display a list of windows and names, in order to use with window-specific focus, see '-focus-window'. \n"
                "-focus-window <name>:  the name of the window to give exclusive focus to. Use -report to discover the correct window name. In case of multiple windows for the same app the largest is selected. \n"
                "-inset <left,top,right,bottom>: the number of pixels to inset the image from the standard outer window bounds, eg -inset 5,40,5,5 \n"
                "-max-fps <n>:          n is the maximum number of frames per second to send to a client, to prevent overload/lag. \n" );
    }
